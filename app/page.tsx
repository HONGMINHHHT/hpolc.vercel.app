...
export default function Home(){
 ...
 function download(){
  const rows:(string|number)[][]=[["TIẾN ĐỘ HỌC TẬP CỦA CÁC LỚP QUẢN LÝ"],["TT","Ngành","Trạm","Lớp","QĐ trúng tuyển","Đối tượng","Số lượng","Ghi chú"]];
  data.forEach((d,di)=>d.groups.forEach((n,gi)=>rows.push([gi?"":di+1,"Luật","Trường Trung cấp Nghề Bình Thạnh","K64T1-Luật",gi?"":`${d.id} ngày ${d.date} - ${d.type} - Danh sách tại trạm: ${d.students} SV`,names[gi],n,d.warn&&gi===0?"Số tại Điều 1 khác danh sách của trạm/lớp":""])));
  rows.push(["","","","","","TỔNG SỐ SINH VIÊN QUẢN LÝ",113,""]);
  const wb=XLSX.utils.book_new();
  const summary=XLSX.utils.aoa_to_sheet(rows);summary["!cols"]=[{wch:5},{wch:16},{wch:38},{wch:16},{wch:55},{wch:16},{wch:12},{wch:42}];
   ...
   <main className="min-w-0 p-4 sm:p-6 lg:p-8">
    ...
    <section className="mb-6 grid gap-4 xl:grid-cols-[1.25fr_.75fr]">
     <div className="rounded-2xl border bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><div><h2 className="font-bold text-[#244c70]">Nhập tài liệu</h2><p className="mt-0.5 text-sm text-[#71879b]">Excel mẫu và các PDF quyết định trúng tuyển</p></div><span className="rounded-full bg-[#edf6ff] px-3 py-1 text-xs font-bold text-[#0e66ad]">Bảo mật trong phiên</span></div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t pt-4"><p className="text-xs leading-5 text-[#758a9c]">Giữ riêng số tại Điều 1 và số sinh viên trong danh sách kèm theo.</p><button onClick={process} disabled={busy||(!excel&&!pdfs.length)} className="shrink-0 rounded-xl bg-[#12a37f] px-4 py-2.5 text-sm font-bold text-white disabled:opacity-40">{busy?"Đang phân tích...":"Phân tích dữ liệu"}</button></div>
     </div>
